...
vim.keymap.set(
  "n",
  "<leader>m",
  ":botright 10split | terminal bash -c 'mvn clean package && java -jar $(find target/*.jar | head -n 1)'<CR>:startinsert<CR>",
  -- ":botright 10split | terminal mvn clean package && java -jar $(ls target/*.jar | head -n 1)<CR>:startinsert<CR>",
